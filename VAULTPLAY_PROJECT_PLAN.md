...
- [x] Settings page visual overhaul — two-card centered column (Account identity + Change Password); glassmorphism cards with gold corner brackets, gradient avatar with initials, eye-toggle inputs (40×40px touch targets), conditional strength meter (weak→warn, fair→gold, strong→ok), real-time match indicator; upgrade `useTheme` to 3-mode (dark/light/system) with live OS tracking and `vp-theme-mode` storage key migration; add `--vp-warn`, `--vp-card-shadow`, `--vp-track` tokens; Appearance panel removed to simplify UX
- [x] Admin page visual overhaul — master-detail CSS grid (360px + `minmax(0,1fr)`, collapses to single column at ≤880px); header with `// administration` eyebrow, Chakra Petch title, stat line showing user + game totals with violet-bright accents; glassmorphism user cards with 42×42 gradient avatar, gold `admin` role badge, IBM Plex Mono email + game count, violet border + glow on selected card, warn-red delete hover; vault rows with 52×70 cover thumbnail (real image or grain+monogram fallback), color-coded status badges (violet/gold/ok), rating for completed games; gold count pills on both panel headers; full keyboard support; `.adm-*` CSS block using `--vp-*` tokens
