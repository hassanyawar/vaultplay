...
**M1 complete** — tagged `M1` on `master`. **M2 complete** — tagged `M2` on `master`. **M3 complete** — tagged `M3` on `master`. **M4 complete** — tagged `M4` on `master`. **M5 complete** — tagged `M5` on `master`. **M6 complete** — JWT auth with httpOnly cookies, per-user vault scoping, change password, admin panel, username support. **M7 complete** — tagged `M7` on `master`. Deployed to Vercel + Railway, dark/light mode, mobile responsive with bottom nav, load-more pagination. **M7.1 complete** — tagged `M7.1` on `master`. Full VP design system polish pass across all pages: auth (aurora bg, glassmorphism card, unified `AuthPage`, password toggle), search (hero, cover-first `GameCard`, aurora atmosphere, redesigned nav), vault (glassmorphism rows, inline notes, filter chips, color-coded stats), discover (glassmorphism rec cards with distinct rank badges, shelf rows, animated SVG radar chart with coverage bars), dashboard (KPI cards with accent glow, animated status bar, SVG area chart for completions, ranked bar chart for genres), settings (two-card centered column — Account identity + Change Password; glassmorphism with gold corners, gradient avatar, eye-toggle inputs, conditional strength meter, match indicator; Appearance panel removed), admin (master-detail CSS grid 360px + 1fr collapsing at ≤880px; glassmorphism user cards with gradient avatars, gold admin role badge, violet selected-state glow, warn-red delete hover; vault rows with cover thumbnails, grain+monogram fallback, color-coded status badges). Global `--vp-*` CSS design tokens, Chakra Petch + IBM Plex Mono fonts, 3-mode theme (dark/light/system) in `useTheme.ts`, `body.app-mode` aurora layers live in `client/src/index.css` + `client/src/App.tsx`.
